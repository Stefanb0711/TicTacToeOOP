...
"""if aktueller_spielstand.spieler_0 == None or aktueller_spielstand.spieler_x == None:
...
class TicTacToe:
    ...
    def benutzereingabe_und_feld_setzen(self, spieler):
        if spieler == "X":
            ...
            if not tic_tac_toe.spieler_x_eingabe.isdigit():
                print("Der eingegebene Wert ist nicht gültig. Versuchen Sie es nochmal")
                #continue
            elif int(tic_tac_toe.spieler_x_eingabe) > 9 or int(tic_tac_toe.spieler_x_eingabe) < 1:
...
while not tic_tac_toe.spiel_zu_ende:
